...
int main(void) {
    ...
    struct node *new = create_node(3, NULL);
    printf("%d\n", new->data);
    free(new); // free takes in a memory address
    // printf("%d\n", new->data); // malloc use after free error
    ...
    // free(head); - this only frees the first node not the entire list
